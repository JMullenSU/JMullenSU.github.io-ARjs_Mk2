...
AFRAME.registerComponent("gesture-detector", {
    ...
    emitGestureEvent(event) {
        ...
        const gestureEnd = previousState && !gestureContinues;
        ...
        if (gestureContinues){
            ...
            if (currentState.spread){
                eventDetail.speadChange = currentState.spread - previousState.spread;
            }
    ...
    getTouchState: function(event){
        ...
        // Convert event.touches to an array, so that reduce can be used
        ...
        // Calculate center for all touches
        const centerPositionRawX = 
            touchList.reduce((sum, touch) => sum + touch.clientX, 0) / touchList.length;

        const centerPositionRawY = 
            touchList.reduce((sum, touch) => sum + touch.clinetY, 0) / touchList.length;

        touchState.positionRaw = { x: centerPositionRawX, y: centerPositionRawY};

        // Scale touch position and spread by average window dimensions
